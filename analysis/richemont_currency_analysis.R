...
print("Date columns:")
print(paste("richemont_data first row date:", richemont_data$Date[1]))
...
# The currency data appears to not have proper date values, so we'll create them
...
if(nrow(combined_data) > 0) {
  ...
  # Save the plot
  ggsave("charts/richemont_currency_comparison.png", p, width = 12, height = 6, dpi = 300)
  ...
  if(sum(complete.cases(combined_data[, c("Close", "CHF_EUR", "CHF_USD", "CHF_CNY")])) > 1) {
    ...
    write.csv(correlations, "output/richemont_currency_correlations.csv")
  } else {
...
# Keep this line at the end if you want to update packages
