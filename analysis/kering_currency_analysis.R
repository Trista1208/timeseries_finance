...
print("Date columns:")
print(paste("kering_data first row date:", kering_data$Date[1]))
...
combined_data <- merge(kering_data, currency_data, by = "Date", all = FALSE)
...
if(nrow(combined_data) > 0) {
  ...
  p <- ggplot(combined_data, aes(x = Date)) +
    geom_line(aes(y = normalized_price, color = "Kering Stock Price"), linewidth = 1) +
    ...
    scale_color_manual(values = c(
      "Kering Stock Price" = "#1f77b4",
      "CHF/EUR" = "#ff7f0e",
      "CHF/USD" = "#2ca02c",
      "CHF/CNY" = "#d62728"
    )) +
    labs(
      title = "Kering Stock Price vs Currency Exchange Rates",
  ...
  # Save the plot
  ggsave("charts/kering_currency_comparison.png", p, width = 12, height = 6, dpi = 300)
  
  # Calculate correlations
  print("Correlations between Kering stock price and currency rates:")
  ...
  if(sum(complete.cases(combined_data[, c("Close", "CHF_EUR", "CHF_USD", "CHF_CNY")])) > 1) {
    ...
    write.csv(correlations, "output/kering_currency_correlations.csv")
  } else {
