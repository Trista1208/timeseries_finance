...
merged_data <- merged_data %>%
  arrange(Date) %>%
  mutate(
    EUR = na.locf(EUR, na.rm = FALSE),
    USD = na.locf(USD, na.rm = FALSE),
...
# Check stationarity
adf_test <- adf.test(ts_richemont)
...
if (adf_test$p.value > 0.05) {
  ...
  adf_test_diff <- adf.test(ts_richemont_diff)
...
cat("\nFitting ARIMA model...\n")
arima_model <- auto.arima(ts_richemont, seasonal = FALSE, stepwise = TRUE, approximation = FALSE)
...
arimax_model <- auto.arima(ts_richemont, xreg = xreg_matrix, seasonal = FALSE)
