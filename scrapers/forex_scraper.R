...
richemont_data <- read.csv("C:\\Users\\vincent schreyer\\PycharmProjects\\timeseries_analysis\\timeseries_finance\\data\\richemont_historical_data.csv") # nolint
...
merged_data <- Reduce(function(x, y) merge(x, y, join = "inner"),
                      list(log_returns_richemont, log_returns_chfeur, log_returns_chfusd, log_returns_chfcny)) # nolint

colnames(merged_data) <- c("Richemont", "CHFEUR", "CHFUSD", "CHFCNY")
...
lm_model <- lm(Richemont ~ CHFEUR + CHFUSD + CHFCNY, data = merged_data)
summary(lm_model)
...
y <- merged_data$Richemont
x <- merged_data[, c("CHFEUR", "CHFUSD", "CHFCNY")]
...
x_weekly <- weekly_returns[, c("CHFEUR", "CHFUSD", "CHFCNY")]
...
x_monthly <- monthly_returns[, c("CHFEUR", "CHFUSD", "CHFCNY")]
